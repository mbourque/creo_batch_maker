...
def nested_generics_from_instance_labels(instance_names: list[str]) -> list[str]:
    """
...
def nested_generics_in_check_inner(inner: str) -> list[str]:
    names: list[str] = []
...
def nested_generics_from_source(raw_xml: str, source_check: str) -> list[str]:
...
def process_xml_file(
    ...
    for job in file_jobs:
        try:
            ...
            nested_names = nested_generics_from_source(out, job.source_check)
            ...
            if not nested_names:
                ...
                ans = str(len(nested_names))
                items = [(name, "Nested family table") for name in nested_names]
