...
- **Thumbnails** — **Thumbnails >** same flow. **Skip** when `.jpg` files already exist in the working folder. **Next >** advances to **Create Report**. Failed models update live the same way (**`creo-batch-timeouts-jpeg3d.txt`**).
...
- After **Thumbnails**, the wizard moves to **Create Report** when ModelCHECK XML and `.jpg` files are present (also rechecked while a batch runs and when you return to the app).
