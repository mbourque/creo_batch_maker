...
- **ModelCHECK** — **Run ModelCHECK >** writes chunk `.dxc` files and launches PowerShell. **Skip** when ModelCHECK result XML (`*.p.xml`, `*.a.xml`, `*.d.xml`) already exists in the working folder. **Waiting…** until `creo-batch-*.dxc` files are gone; then **Next >** advances one step. If any models fail (expected outputs never appear before the inactivity timeout), a red **Failed (N):** line under the progress bar lists them as the batch run writes **`creo-batch-timeouts-modelcheck.txt`** (comma-separated; long lists truncate with “… and N more”). That log is removed when you start a new ModelCHECK run or use **Start over…**.
