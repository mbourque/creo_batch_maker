...
- Open Batch starts the runner in PowerShell when batch artifacts exist: for **ModelCHECK** / JPEG tasks, `creo-batch-run.ps1` and `creo-batch-*.dxc` in the working folder; for **Scan Templates**, `templates\creo-batch-run.ps1` and `templates\templates.dxc` (run GO again after a finished run, because the runner deletes the `.dxc` file). When Open Batch launches successfully, **Task** advances: **Scan Templates** → **ModelCHECK** → JPEG 3D (if that task is in the list).
