...
- **Create Report** — **Create Report** merges ModelCHECK XML into **`master.xml`**, removes leftover runner `.ps1` files, then builds **`index.html`** in the background (score dashboard, batch statistics, filterable check sidebar, per-model detail). When **`index.html`** already exists, **Open Report** opens it in your browser without rebuilding. Only dialog after a new build: **Open in browser?**
