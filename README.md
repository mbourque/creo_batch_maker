...
- **Batch settings…** — one dialog for ModelCHECK and Thumbnails batch options (re-run **GO** after changing so a new runner script is generated):
  - **Chunk size** — models per `creo-batch-N.dxc` chunk (**1–100**, default **10**). JSON key: `chunk_size`.
  - **Output wait timeout** — seconds with no new output file before a chunk times out (**whole number ≥ 60**, default **120**). JSON key: `output_timeout_sec`.
  - **Xtop gone timeout** — seconds to wait for `xtop.exe` to return after it exits mid-chunk (**XTOP GONE**; **whole number ≥ 5**, default **20**). JSON key: `xtop_timeout_sec`. Waiting for `xtop` to **start** after launch is fixed at **60 s** (**XTOP NEVER STARTED**).
...
Opens bundled ModelCHECK files from the app’s `configs\` folder in Notepad (available for all tasks). This menu used to be named **Settings**; it was renamed to **Configuration** so **Settings** could hold app-wide options (batch settings, automatic mode, debug).
